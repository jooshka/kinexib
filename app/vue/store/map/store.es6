  ...
  actions: {
    ...
    set_active_city({dispatch, commit, state}, [index, by]) {
      ...
      if (idx >= 0 ) {
        ...
        if (!state.country_map_exist)
          dispatch('init_country_map');
        dispatch('fetch_shops');
      }
    },

    init_country_map({state, dispatch}) {
      state.country_map_exist = true;
    ...
    init_city_map({getters, commit, dispatch}) {
      let timerId = setInterval(function() {
        if (getters.isYandexReady) {
          ...
          let city_map = new ymaps.Map("cityMap",
            {
              center: [55.76, 37.64],
              zoom: 14,
              controls: []
